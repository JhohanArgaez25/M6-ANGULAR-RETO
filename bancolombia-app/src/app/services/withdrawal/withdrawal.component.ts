...
export class WithdrawalComponent {
  ...
  ngOnInit(accountId: number, valor: number): void {
    //this.withdraw(accountId, valor);
  }

  fetchWithdraw(accountId: number, valor: number): void {
    this.apiService.withdraw(accountId, valor).subscribe(
      data => {
      ...
      error => {
        ...
        this.saldo = null;
        console.error('Error al realizar un retiro', error);
      }
    );
  }
  ...
  onSubmit(): void {
    if (this.transactionForm.valid) {
      ...
      this.fetchWithdraw(accountId, valor);
    } else {
